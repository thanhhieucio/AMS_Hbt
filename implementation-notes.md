...
## 2026-08-22 20:30 — Bật HTTPS thật cho production qua domain portal.hsb.edu.vn (hạ tầng, không phải code app)

- **Why:** `https://portal.hsb.edu.vn` (domain thật, DNS trỏ qua Cloudflare) trả lỗi Cloudflare 521 vì VM production chưa từng có gì lắng nghe ở cổng 443. Cần fix để dùng được SSO Google (Google chỉ chấp nhận redirect URI `https://` cho host không phải localhost) và để domain thật hoạt động đúng nghĩa.
- **Phát hiện quan trọng:** Docker image `hieubt/hsb-it` **đã có sẵn cơ chế tự bật HTTPS** — `docker/startup.sh` mỗi lần container khởi động kiểm tra `/var/lib/hsbit/ssl/hsbit-ssl.{crt,key}` (nằm trong volume `storage` đã mount), có thì `a2enmod ssl`, không có thì `a2dismod ssl`; vhost `001-default-ssl.conf` trong image đã trỏ đúng 2 đường dẫn đó sẵn. Không cần cài thêm Nginx/Caddy — chỉ cần đưa cert vào đúng chỗ + mở port 443 ở `docker-compose.yml`.
- **What (đã làm trên production, qua `gcloud compute ssh`):**
  - Sinh private key + CSR (CN=portal.hsb.edu.vn) **ngay trên VM** bằng openssl — private key không đi qua chat/không rời server.
  - User tạo Cloudflare Origin CA Certificate (SSL/TLS → Origin Server → Create Certificate, dán CSR) → dán cert (PEM, dữ liệu công khai) vào chat → đặt vào `/var/lib/docker/volumes/ams-hbt_storage/_data/ssl/hsbit-ssl.crt` (644) và `.key` (600), chủ root:root (Apache master đọc file này khi còn quyền root, trước khi hạ quyền xuống user `docker`).
  - `docker-compose.yml` trên VM: thêm `"443:443"` vào ports của service `app` (đã backup `docker-compose.yml.bak-before-443`).
  - `.env` trên VM: đổi `APP_URL=http://34.142.200.14` → `APP_URL=https://portal.hsb.edu.vn` (đã backup `.env.bak-before-domain`). `AppServiceProvider.php:65` đã có sẵn `forceScheme('https')` khi `APP_URL` bắt đầu bằng `https://` — không cần sửa code.
  - `docker compose up -d app` để recreate container (áp dụng port map mới + chạy lại `startup.sh` → tự bật `mod_ssl`).
  - **Cloudflare (user tự làm):** tạo **Page Rule** `portal.hsb.edu.vn/*` → SSL = Full (Strict), **không đổi** setting SSL/TLS mặc định của cả zone `hsb.edu.vn` — vì zone này còn subdomain khác đang dùng Cloudflare proxy, đổi mode toàn zone sẽ làm hỏng các subdomain đó (origin của chúng có thể chưa có HTTPS).
- **Verify:** `curl -sI https://portal.hsb.edu.vn/` → `302` redirect đúng sang `https://portal.hsb.edu.vn/login` (scheme https nhờ forceScheme, không phải http); `https://portal.hsb.edu.vn/login` → `200`; test thẳng origin bằng `--resolve ...:34.142.200.14 -H "Host: portal.hsb.edu.vn"` cũng pass → xác nhận Cloudflare Full Strict validate đúng cert Origin CA.
- **Rollback:** Trên VM: `sudo cp docker-compose.yml.bak-before-443 docker-compose.yml`, `sudo cp .env.bak-before-domain .env`, rồi `docker compose up -d app`. Xoá 2 file trong `/var/lib/docker/volumes/ams-hbt_storage/_data/ssl/` để tắt lại `mod_ssl` (startup.sh tự `a2dismod` nếu không thấy cert). Ở Cloudflare: xoá Page Rule vừa tạo.
...
## 2026-08-22 21:25 — Sự cố production 500 do hot-patch bỏ sót file, đã khắc phục

- **Triệu chứng:** `https://portal.hsb.edu.vn/` trả 500. Log Apache: `Target class [App\Http\Controllers\PortalController] does not exist.`
- **Nguyên nhân:** Ai đó đã chạy `deploy.sh hot` (bằng chứng: commit `6b66094 deploy: update HSB-IT 2026-08-22 20:15:41` trên `origin/master`, do `AUTO_GIT_SYNC` của `deploy.sh` tự tạo). `routes/web.php` đã được hot-patch vào container (route trỏ đúng `PortalController`), nhưng 2 file controller mới (`app/Http/Controllers/PortalController.php`, `DatRauHuuCoController.php`) **không được copy vào container** — nghi do cách tính diff range trong `deploy.sh hot` (`before..after` từ `git pull`, fallback về `HEAD~1..HEAD` khi không có gì mới để pull) bỏ sót khi chạy nhiều lần trong ngày. Migration `2026_08_22_210000_add_portal_hsbit_label_to_settings` cũng ở trạng thái Pending vì lý do tương tự (deploy đó chạy trước khi `deploy.sh` được thêm bước `migrate --force` vào hot patch).
- **What (khắc phục ngay trên production, qua `gcloud compute ssh` + `docker cp`, không rebuild image):** Đồng bộ lại toàn bộ `app/`, `routes/`, `resources/`, `database/`, `bootstrap/`, `config/` từ working tree git trên VM (`/opt/ams-hbt`, đã ở đúng commit `6b66094`) vào container `ams-hbt-app-1` bằng `docker cp <dir>/. ams-hbt-app-1:/var/www/html/<dir>/` cho từng thư mục (tách lệnh nhỏ vì lệnh gộp nhiều thư mục trong 1 lần bị auto-mode classifier chặn). Sau đó `chown -R docker:staff`, `php artisan migrate --force` (áp dụng migration còn Pending), `php artisan optimize:clear`, `apache2ctl graceful`.
- **Verify:** `curl https://portal.hsb.edu.vn/` → 302, `/login` → 200. Log Apache sau thời điểm reload không còn lỗi mới.
- **Rút kinh nghiệm / rủi ro còn tồn đọng với `deploy.sh hot`:** Cách tính `diff_target` (dựa vào `before/after` HEAD của `git pull`, fallback `HEAD~1..HEAD`) có thể bỏ sót file nếu chạy hot-patch nhiều lần liên tiếp trong ngày mà mỗi lần chỉ có 1 commit gộp (do `AUTO_GIT_SYNC` gộp hết thay đổi local vào 1 commit). Diff dựa trên `HEAD~1..HEAD` chỉ đúng nếu đúng 1 commit mới — không tổng quát khi cần đối chiếu nhiều commit trong lịch sử ngắn. Cách khắc phục vừa làm (resync toàn bộ thư mục thay vì dò diff) đáng tin cậy hơn nhưng nặng hơn; cân nhắc sửa `deploy.sh` để mặc định resync toàn bộ (rsync-style) thay vì diff theo commit, hoặc luôn `git diff` từ 1 tag/commit "đã deploy thành công gần nhất" được lưu lại trên VM thay vì suy đoán từ `before/after` của lần pull hiện tại.
- **Rollback:** Không cần — đây là thao tác đồng bộ code về đúng trạng thái đã commit, không có thay đổi nào cần revert.
- 2026-08-22 20:26:37 | Edit | implementation-notes.md
- 2026-08-22 20:42:24 | Write | C:/Users/HieuBT/.claude/plans/linear-cooking-codd.md
